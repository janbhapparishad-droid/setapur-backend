...
replacement = r'''async function ensureCalendarEventsTable() {
  await pool.query(
...
new_code = re.sub(r'async function ensureCalendarEventsTable\(\) \{[\s\S]*?ensureCalendarEventsTable\(\)\.catch', replacement + '\nensureCalendarEventsTable().catch', code)

with open('src/server.js', 'w', encoding='utf-8') as f:
    f.write(new_code)

print("Replaced whole function")
