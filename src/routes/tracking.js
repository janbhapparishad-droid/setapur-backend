...
// Heartbeat - called every 60s while app is open
...
// Single event tracking (optional, we mostly use batch)
...
router.post('/events/batch', async (req, res) => {
  try {
    ...
    if (!events || !Array.isArray(events)) return res.status(400).json({ error: 'Invalid events array' });
