...
/* removed old global fix block *//* removed old global fix block */function pgNum(x) { const n = Number(x); return Number.isFinite(n) ? n : 0; }
  ...
  try {
    ...
    if (e.submittedBy) {
      pushNotification(e.submittedBy, {
        ...
        body: `${e.category} ï¿½ ?${e.amount} (pending approval)`,
    ...
    if (!alreadyApproved) {
      ...
      if (donorUser) {
        pushNotification(donorUser, {
          ...
          body: `Receipt: ${rcOut || 'N/A'} ï¿½ Event: ${out.category} ï¿½ Amount: ?${pgNum(out.amount)}`,
...
// removed stray top-level code:   let list = rows.map(r => r.id);
// removed stray top-level code:   let idx = list.indexOf(Number(folderId));
  if (idx === -1) return;
  if (typeof newIndex === 'number' && Number.isFinite(newIndex)) {
    const it = list.splice(idx,1)[0];
    list.splice(Math.max(0, Math.min(newIndex, list.length)), 0, it);
...
// removed stray top-level code: // removed stray top-level await: /* removed top-level await */   for (let i=0;i<list.length;i++) /*await pool.query(*/'UPDATE analytics_folders SET order_index=$1 WHERE id=$2', [i, list[i]]);
// removed stray top-level brace: }

// removed stray top-level code:   let list = rows.map(r => r.id);
// removed stray top-level code:   let idx = list.indexOf(Number(eventId));
  if (idx === -1) return;
  if (typeof newIndex === 'number' && Number.isFinite(newIndex)) {
    const it = list.splice(idx,1)[0];
    list.splice(Math.max(0, Math.min(newIndex, list.length)), 0, it);
...
// removed stray top-level code: // removed stray top-level await: /* removed top-level await */   for (let i=0;i<list.length;i++) /*await pool.query(*/'UPDATE analytics_events SET order_index=$1 WHERE id=$2', [i, list[i]]);
// removed stray top-level brace: }
