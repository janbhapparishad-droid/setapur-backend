...
const pool = new Pool({
  ...
  ssl: /sslmode=require|neon|render|amazonaws|\.neon\.tech/i.test(process.env.DATABASE_URL || "")
...
/* Resolve folder numeric id from id/uuid/slug/name */
...
router.post('/folders', async (req,res)=>{
  try{
    ...
    // slug if column exists, ensure uniqueness
    let hasSlug = false;
    try {
...
router.put('/folders/:id', async (req,res)=>{
  try{
    ...
    if (nmIn){
      fields.push('name=/* Enable/disable folder */
...
/* List events of folder (resolve param flexibly) */
...
/* Create event (resolve folder by id/uuid/slug/name) */
...
router.put('/events/:id', async (req,res)=>{
  try{
    ...
    if (nmIn){ fields.push('name=/* Enable/disable event */
...
module.exports = router;
 + (i++)); vals.push(nmIn);
      ...
      if (hasSlug){
        ...
        for (let j=2;j<100;j++){
          const { rows } = await pool.query('SELECT 1 FROM analytics_folders WHERE slug=' AND id<>$2 LIMIT 1',[sl,id]);
          if(!rows.length) break; sl = slugifyLite(nmIn) + '-' + j;
        }
        fields.push('slug=/* Enable/disable folder */
router.post('/folders/:id/enable', async (req,res)=>{
  try{
    const { id } = req.params; const en = b(req.body?.enabled);
    const { rows } = await pool.query('UPDATE analytics_folders SET enabled=$1 WHERE id=$2 RETURNING *', [en, id]);
...
module.exports = router;
 + (i++)); vals.push(sl);
      ...
      try {
        ...
        if (rows.length){ fields.push('folder_name=/* Enable/disable folder */
router.post('/folders/:id/enable', async (req,res)=>{
  try{
    const { id } = req.params; const en = b(req.body?.enabled);
    const { rows } = await pool.query('UPDATE analytics_folders SET enabled=$1 WHERE id=$2 RETURNING *', [en, id]);
...
module.exports = router;
 + (i++)); vals.push(nmIn); }
    ...
    if (enabled !== undefined){ fields.push('enabled=/* Enable/disable folder */
router.post('/folders/:id/enable', async (req,res)=>{
  try{
    const { id } = req.params; const en = b(req.body?.enabled);
    const { rows } = await pool.query('UPDATE analytics_folders SET enabled=$1 WHERE id=$2 RETURNING *', [en, id]);
...
module.exports = router;
 + (i++)); vals.push(b(enabled)); }

    if (!fields.length) return res.status(400).json({ error:'No changes' });
    vals.push(id);

    const sql = 'UPDATE analytics_folders SET ' + fields.join(', ') + ' WHERE id=/* Enable/disable folder */
router.post('/folders/:id/enable', async (req,res)=>{
  try{
    const { id } = req.params; const en = b(req.body?.enabled);
    const { rows } = await pool.query('UPDATE analytics_folders SET enabled=$1 WHERE id=$2 RETURNING *', [en, id]);
...
module.exports = router;
 + (i++)); vals.push(nmIn); }
    if (enabled !== undefined){ fields.push('enabled=/* Enable/disable event */
router.post('/events/:id/enable', async (req,res)=>{
  try{
    const { id } = req.params; const en = b(req.body?.enabled);
    const { rows } = await pool.query('UPDATE analytics_events SET enabled=$1 WHERE id=$2 RETURNING *', [en, id]);
...
module.exports = router;
 + (i++)); vals.push(b(enabled)); }
    if (showDonationDetail !== undefined){ fields.push('show_donation_detail=/* Enable/disable event */
router.post('/events/:id/enable', async (req,res)=>{
  try{
    const { id } = req.params; const en = b(req.body?.enabled);
    const { rows } = await pool.query('UPDATE analytics_events SET enabled=$1 WHERE id=$2 RETURNING *', [en, id]);
...
module.exports = router;
 + (i++)); vals.push(b(showDonationDetail)); }
    if (showExpenseDetail !== undefined){ fields.push('show_expense_detail=/* Enable/disable event */
router.post('/events/:id/enable', async (req,res)=>{
  try{
    const { id } = req.params; const en = b(req.body?.enabled);
    const { rows } = await pool.query('UPDATE analytics_events SET enabled=$1 WHERE id=$2 RETURNING *', [en, id]);
...
module.exports = router;
 + (i++)); vals.push(b(showExpenseDetail)); }

    if (!fields.length) return res.status(400).json({ error:'No changes' });
    vals.push(id);

    const sql = 'UPDATE analytics_events SET ' + fields.join(', ') + ' WHERE id=/* Enable/disable event */
router.post('/events/:id/enable', async (req,res)=>{
  try{
    const { id } = req.params; const en = b(req.body?.enabled);
    const { rows } = await pool.query('UPDATE analytics_events SET enabled=$1 WHERE id=$2 RETURNING *', [en, id]);
