...
async function run() {
  // Clear the existing seeded ones (to avoid duplicates, since we didn't add a unique constraint on title+date)
  await pool.query('TRUNCATE TABLE calendar_events RESTART IDENTITY');
  
  let count = 0;
  for (const f of festivals) {
    await pool.query('INSERT INTO calendar_events (title, event_date, description) VALUES (, , )', [f.title, f.date, f.desc]);
    count++;
  }
  console.log(Inserted  festivals!);
  pool.end();
}
