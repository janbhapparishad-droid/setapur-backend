...
/* removed top-level await */   for (let i=0;i<list.length;i++) /*await pool.query(*/'UPDATE analytics_folders SET order_index=$1 WHERE id=$2', [i, list[i]]);
}
...
/* removed top-level await */   for (let i=0;i<list.length;i++) /*await pool.query(*/'UPDATE analytics_events SET order_index=$1 WHERE id=$2', [i, list[i]]);
}
